...
if [[ -n "$REPO_PATH" ]]; then
    ...
    if [[ -f "$source_helper" ]]; then
        if [[ ! -f "$vault_scripts_dir/cora-session-start.sh" ]]; then
            ...
            cp "$source_helper" "$vault_scripts_dir/cora-session-start.sh"
            chmod +x "$vault_scripts_dir/cora-session-start.sh"
            ok "Installed cora-session-start.sh into vault scripts dir"
        else
            log "cora-session-start.sh already present in vault scripts dir"
        fi
    else
        warn "cora-session-start.sh not found in $REPO_DIR/scripts — session-start hooks will not work"
    fi

    # ── 6. Gitignore agent files in the repo ──────────────────────────────────
...
if [[ -n "$REPO_PATH" ]]; then
    ...
    echo ""
    echo "  SessionStart hooks installed for:"
    echo "    Claude Code    → $REPO_PATH/.claude/settings.json"
    echo "    Codex          → $REPO_PATH/.codex/hooks.json"
    echo "    Gemini CLI     → $REPO_PATH/.gemini/hooks.json"
    echo "    GitHub Copilot → $REPO_PATH/.github/hooks/cora-session-start.json"
fi
